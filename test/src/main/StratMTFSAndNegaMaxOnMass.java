...
public class StratMTFSAndNegaMaxOnMass extends Grid {
	...
	public StratMTFSAndNegaMaxOnMass(String str, int type, int myColor) {
	...
	private int negaMax(int lvl, int lvlsDone, int alpha, int beta){
		...
		if(lvl == 0){
			...
			for(Long move: moves){
				StratMTFSAndNegaMaxOnMass advGrid = new StratMTFSAndNegaMaxOnMass(this, move, true);
	...
	protected long getBestMove(int lvl) {
		...
		for(Long move: moves){
			if(center == min)
				...
				beta = center;
			StratMTFSAndNegaMaxOnMass advGrid = new StratMTFSAndNegaMaxOnMass(this, move, true);
			center = - advGrid.negaMax(lvl, 0, -beta, -beta+1);
			if(center  <beta){
				max = center;
			}
			else{
