...
public StratMass1(String str, int type, boolean myColor) {
...
int val = -((StratMass1) advGrid).getBestMove(lvl - 1, 0, -beta,
-alpha);
...
for (int i = 0; i < 64; ++i) {
if ((1L << i & mPionsAdv) != 0) {
gap += 1000 - Math.sqrt(Math.pow(massX - i % 8, 2)
+ Math.pow(massY - i / 8, 2));
...
for (Long mvt : moves) {
Grid advGrid = new StratMass1(this, mvt, true);
int val = -((StratMass1) advGrid).getBestMove(lvl - 1,
lvlsDone + 1, -beta, -alpha);
if (val > best) {
best = val;
if (best > alpha) {
alpha = best;
