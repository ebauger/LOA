...
public class StratNMMassThreadFixed extends Grid {
	
	

	public StratNMMassThreadFixed(String str, int type, int myColor) {
		super(str, type, myColor);
	}
	
	public StratNMMassThreadFixed(Grid g, long mvt, boolean inverse) {
		super(g, mvt, inverse);
	}
	...
	private int negaMax(int lvl, int lvlsDone, int alpha, int beta){
		...
		if(lvl == 0){
			...
			for(Long move: moves){
				StratNMMassThreadFixed advGrid = new StratNMMassThreadFixed(this, move, true);
	...
	protected long getBestMove(int lvl) {
		...
		ExecutorService es = Executors.newFixedThreadPool(NB_THREADS+1);
		
		
		long bestMvt = stackMvts.peek();
		
		LinkedBlockingDeque<WorkerRes> resQueue = new LinkedBlockingDeque<WorkerRes>(100);
		synchronized (resQueue) {
		 ...
		 for(int i= 0; i < NB_THREADS && !stackMvts.isEmpty(); ++i){
		    	...
		    	GridWorker worker = new GridWorker(this, lvl, INT_MIN_VALUE, Integer.MAX_VALUE ,stackMvts.pop(), resQueue);
		    	//workers.add(worker);
		    	es.execute(worker);
		...
		Future<Long> best = es.submit(new GridHandler(this, resQueue, stackMvts, es, lvl));
		try {
			bestMvt = best.get();
	...
	private class GridHandler implements Callable<Long>{
		...
		Grid grid;
		int lvl;
		private GridHandler(Grid g,LinkedBlockingDeque<WorkerRes> queue, Stack<Long> moves, ExecutorService es, int lvl){
			this.grid = g;
			this.queue = queue;
			this.moves = moves;
			this.es = es;
		...
		public Long call() throws Exception {
			...
			bigloop: while (!done) {
				...
				if (!moves.isEmpty()) {
					...
					/*((GridWorker) wr.th)
							.setMove(grid, alpha, beta, moves.pop());*/
					es.execute(new GridWorker(grid, lvl, alpha, beta, moves.pop(), queue));
	...
	class GridWorker extends Thread{
		  ...
		  GridWorker(Grid grid, int lvl, int alpha, int beta, long mvt, LinkedBlockingDeque<WorkerRes> queue ) {
			  super();
		...
		public void run() {
			...
			for (Long move : moves) {
				StratNMMassThreadFixed advGrid = new StratNMMassThreadFixed(
						grid, move, true);
