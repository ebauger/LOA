...
public class StratSimpleNegaMaxMass extends Grid {
	...
	public StratSimpleNegaMaxMass(String str, int type, boolean myColor) {
	...
	private int negaMax(int lvl, int lvlsDone, int alpha, int beta) {
		...
		if (lvl == 0) {
			...
			for (Long move : moves) {
				StratSimpleNegaMaxMass advGrid = new StratSimpleNegaMaxMass(
						this, move, true);
				int val = -advGrid
	...
	protected long getBestMove(int lvl) {
		...
		for (Long move : moves) {
			StratSimpleNegaMaxMass advGrid = new StratSimpleNegaMaxMass(this,
					move, true);
		 ...
		 * int nbMoves = moves.size(); loopFEM: for(int i = 0; i<nbMoves &&
		 * min<max; ++i){ if(center == min) beta = center+1; else beta = center;
		 * StratBlockAndMass advGrid = new StratBlockAndMass(this, moves.get(i),
		 * true); center = - advGrid.negaMax(lvl, 0, -beta, -alpha);
		 * if(center<beta) max = center; else min = center;
		 */
